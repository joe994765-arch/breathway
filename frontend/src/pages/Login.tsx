...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(msg.message || msg.error || "Login failed");
      }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = await res.json().catch(() => ({ error: "Signup failed" }));
        throw new Error(msg.message || msg.error || "Signup failed");
      }
      const user = await res.json();
